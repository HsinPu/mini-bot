...
- CLI 指令會安裝到 `.venv/bin/minibot`
- 啟用虛擬環境後，直接執行 `minibot` 就會使用這個版本
- 若使用 `python3 -m pip install --user .`，通常會安裝到 `~/.local/lib/python3.x/site-packages/`，CLI 在 `~/.local/bin/minibot`
- 若直接安裝到系統 Python，通常會在系統的 `site-packages`，CLI 常見位置是 `/usr/local/bin/minibot`

可用以下指令確認實際安裝位置：

```bash
which minibot
python -c "import minibot; print(minibot.__file__)"
```
...
```bash
# 從目前啟用的環境移除套件
python -m pip uninstall minibot
...
- 若是用 `--user` 安裝，執行 `python3 -m pip uninstall minibot`
- 若是安裝到系統 Python，可能需要使用 `sudo python3 -m pip uninstall minibot`
