...
Path layout:
- app home: ~/.minibot
- bootstrap files: ~/.minibot/bootstrap/*.md
- memory: ~/.minibot/memory/{chat_id}/MEMORY.md
- default skills: ~/.minibot/skills/*/SKILL.md
- tool workspace: ~/.minibot/workspace
"""
...
def get_app_home(app_home: str | Path | None = None) -> Path:
    """Resolve the mini-bot app home directory."""
    path = Path(app_home).expanduser() if app_home else MINIBOT_HOME
...
def migrate_legacy_bootstrap(app_home: str | Path | None = None, silent: bool = False) -> list[str]:
    """Copy legacy bootstrap markdown files into ~/.minibot/bootstrap."""
...
def migrate_legacy_memory(app_home: str | Path | None = None, silent: bool = False) -> list[str]:
    """Copy legacy memory files into ~/.minibot/memory/{chat_id}/MEMORY.md."""
    home = get_app_home(app_home)
...
def sync_templates(app_home: str | Path | None = None, silent: bool = False) -> list[str]:
    """Sync bundled templates into ~/.minibot app directories."""
    home = get_app_home(app_home)
    bootstrap_dir = get_bootstrap_dir(home)
    memory_dir = get_memory_dir(home)
    ...
    try:
        ...
        templates_root = pkg_files("minibot") / "templates"
        skills_root = pkg_files("minibot") / "skills"
    except Exception:
        return added
