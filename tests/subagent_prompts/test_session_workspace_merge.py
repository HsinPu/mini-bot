...
def test_session_only_subagent_is_listed_and_loaded(tmp_path: Path) -> None:
    ...
    chat_ws = get_chat_workspace("telegram:t1", workspace_root=ws_root)
    session_dir = chat_ws / "subagent_prompts"
    ...
    merged = get_all_subagents(app_home, session_workspace=chat_ws)
    assert "session-only" in merged
    assert merged["session-only"] == "from session"
    assert "Only in session." in load_prompt("session-only", app_home=app_home, session_workspace=chat_ws)
...
def test_session_file_overrides_global_for_same_id(tmp_path: Path) -> None:
    ...
    ws_root = app_home / "workspace"
    chat_ws = get_chat_workspace("telegram:t2", workspace_root=ws_root)
    session_dir = chat_ws / "subagent_prompts"
    session_dir.mkdir(parents=True, exist_ok=True)
    ...
    body = load_prompt("writer", app_home=app_home, session_workspace=chat_ws)
